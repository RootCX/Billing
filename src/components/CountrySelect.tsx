...
const COUNTRY_MAP = new Map(ISO_COUNTRIES.map((c) => [c.code, c.name]));
export const countryName = (code: string) => COUNTRY_MAP.get(code) ?? code;
...
export function CountrySelect({ value, onChange, placeholder = "Search country…", className }: Props) {
  ...
  return (
    <div ref={ref} className={cn("relative", className)}>
      <div className="relative">
        <Input
          value={open ? query : (value ? `${value} — ${countryName(value)}` : "")}
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2">
          {value && !open
            ? <button type="button" onClick={(e) => { e.stopPropagation(); onChange(""); setOpen(false); }} className="text-muted-foreground hover:text-foreground transition-colors">
                <IconX className="h-3.5 w-3.5" />
              </button>
            : <IconChevronDown className="h-3.5 w-3.5 text-muted-foreground pointer-events-none" />
          }
        </div>
      </div>

      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[220px] rounded-md border bg-popover shadow-md overflow-hidden">
          <div className="max-h-56 overflow-y-auto">
            {filtered.length === 0
              ...
              : filtered.map((c) => (
                  ...
                  >
                    <span className="font-mono text-xs text-muted-foreground w-6 shrink-0">{c.code}</span>
                    <span>{c.name}</span>
                  </button>
                ))
            }
